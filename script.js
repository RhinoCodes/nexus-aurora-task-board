const { createClient } = supabase;
    ...
    function waitForElement(){
      if (supabase.auth.session()!==null) {
          ...
          document.getElementById("signin").outerHTML = '<img onclick="supabase.auth.signOut(); location.reload()" src="'+supabase.auth.session().user.user_metadata.avatar_url+'" class="rounded-full" style="width:1.5%; height: 1.5%;">';
      } 
      else{
        timesrun++;
        if (timesrun != 10) {
